...
### Limitations
Current assessment relies primarily on informal user feedback from one evaluator, so a formal study with 20 or more participants would strengthen the claims. The benchmark documents are also concentrated in academic and technical domains, which means evaluation on business, creative, and other document types could reveal generalization gaps. In addition, Claude API non-determinism means repeated calls can produce varying summaries for identical inputs, which complicates reproducibility testing. Finally, the current evaluation does not include direct baseline comparisons against alternative tools such as the Google Docs version history interface, GitHub diffs, or standalone diff viewers.

### Recommended Future Evaluation
A stronger future evaluation plan should include a controlled user study that compares task completion time and comprehension between CoTrace summaries and manual diff reading. It should also include formal precision and recall testing for pattern classification on at least 100 manually labeled examples, along with multi-domain validation on business documents, creative writing, technical code, and legal documents. Comparative benchmarking against competing approaches, including syntax-only diffs, line-count statistics, and LLM-agnostic baselines, would further clarify CoTrace's relative strengths and weaknesses. In addition, automated metrics such as BLEU or ROUGE could be used to compare generated summaries with human-written reference summaries. Finally, a longitudinal usability study over several weeks of real usage would provide stronger evidence about engagement, trust, and sustained summary utility.
